class Solution {
    public int rangeBitwiseAnd(int left, int right) {
        ...
        // Optimization: Based on intuition above, the only bits that will be 1s are prefixes
            // Thus, if our range is big enough, don't need to worry about smaller bits
            // Algo:
                // Continuously shift left and right down
                // Compare both numbers (shifted) and see if they're not equal
                    // If so, than the numbers exceed the range of the smaller bits and guarentee there is a 0 in that range
                    // However, if the numbers are equal, then we found our AND – Just shift it back by the same amount
                // Only add to solution if we don't guarentee it
        int i = 0;
        while (left != right) {
            left >>= 1;
            right >>= 1;

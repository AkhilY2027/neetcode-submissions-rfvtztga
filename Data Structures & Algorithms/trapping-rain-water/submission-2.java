class Solution {
    public int trap(int[] height) {
        ...
        // Optimization: Two pointer
            // Have a left pointer on very left of array and right on very right
            // On each iteration, shift the pointer that is lesser – because we only need min(l, r)
                // Calculate the water stored at the position that is shifted
        if (height == null || height.length == 0) return 0;
        int lPos = 0, rPos = height.length - 1;
        int lMax = height[lPos], rMax = height[rPos];
        int sol = 0;
        while (lPos < rPos) {
            // Shift the minimum
            if (lMax < rMax) {
                lPos++;
                lMax = Math.max(lMax, height[lPos]);
                sol += lMax - height[lPos]; // Because we update leftMax before computation, will always be positive
            }
            else {
                rPos--;
                rMax = Math.max(rMax, height[rPos]);
                sol += rMax - height[rPos];
            }
        }
        return sol;
    }

class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        // Via math
            // Know that we must space out the most frequent characters – will count those spaces
                // In addition, add the number of characters that share that max frequency at the end (tail of cycle schedule)
        int[] count = new int[26];
        for (char c : tasks) {
            count[c - 'A']++;
        }
        ...
        return Math.max(tasks.length, (maxCount - 1) * (n + 1) + maxf);
    }
}

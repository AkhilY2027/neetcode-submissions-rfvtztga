class Solution {
    ...
    public boolean checkInclusion(String s1, String s2) {
        ...
        // Optimization: Use a HashTable
            ...
            // Actually O(26 * n) since the max m can be is 26 (letters of alphabet)
                // Technically can be better using a sliding window - but this is essentially one
        ...
        // Part 2: Use a shortcut to declare the number of variables that match
            // Within the sliding window, instead of directly checking the arrays/hashmaps, update this variable
            // Only this variable is equal to the number of characters within s1, then we're good
                // Correction: Since matches is also counting number of letters that are 0, matches must be equal to 26
        ...
        for (int i = s1.length(); i < s2.length(); i++) {
            // Get the letter before and after?
                // Must separate - Could have the same letter before and after but matches must still update properly
            int before = s2.charAt(i - s1.length()) - 'a';
            s2Count[before]--;

            // Update
            if (s1Count[before] == s2Count[before])
                matches++;
            else if (s1Count[before] == (s2Count[before] + 1))
                matches--; // Only decrease if it was a jump
            
            int after = s2.charAt(i) - 'a';
            s2Count[after]++;

            if (s1Count[after] == s2Count[after])
                matches++;
            else if (s1Count[after] == (s2Count[after] - 1))
                matches--; // Only decrease if it was a jump
